...
use sp_core::U512;
use sp_runtime::{
    traits::{One, Saturating, AccountIdConversion, CheckedAdd},
...
pub mod pallet {
    ...
    pub(super) type Deposit<T: Config> = StorageMap<_, Identity, NftIdOf<T>, BalanceOf<T>>;

    /// Nft's Metadata
    #[pallet::storage]
    pub(super) type NftMetaStore<T: Config> =
        StorageMap<_, Twox64Concat, NftIdOf<T>, NftMetaFor<T>>;

    /// Did's preferred Nft.
    #[pallet::storage]
    #[pallet::getter(fn preferred_nft_of)]
    pub(super) type PreferredNft<T: Config> =
        StorageMap<_, Identity, T::DecentralizedId, NftIdOf<T>>;
        ...
        pub fn claim(origin: OriginFor<T>, kol: T::DecentralizedId) -> DispatchResult {
            ...
            if meta.owner == did {
                let minted_block_number =
                    <Date<T>>::get(&nft_id).ok_or(Error::<T>::NotExists)?;
                ensure!(
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn get_or_create_preferred_nft(kol: &T::DecentralizedId) -> Result<NftIdOf<T>, DispatchError> {
